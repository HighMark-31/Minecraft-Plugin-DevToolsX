...
public class PluginReloadCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("devtools.pluginreload")) {
            sender.sendMessage("§cYou don't have permission: §7devtools.pluginreload");
            return true;
        }

        if (args.length != 1) {
            sender.sendMessage("§eUsage: §f/pluginreload <plugin>");
            return true;
        }
        ...
        try {
            ...
            if (plugin.isEnabled()) {
                sender.sendMessage(ChatColor.GREEN + "✓ Plugin " + plugin.getName() + " reloaded successfully!");
                sender.sendMessage(ChatColor.GRAY + "Version: " + plugin.getDescription().getVersion());
            } else {
                sender.sendMessage(ChatColor.RED + "✗ Error reloading plugin!");
            }
